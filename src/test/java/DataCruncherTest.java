...
public class DataCruncherTest {
    ...
    //task Bonus - optional
    @Test
    public void getRiskFraudFigureTest() throws Exception {
        //This is a weird warning error in regards to the bonus, it says it can't access transaction, but it can and it passes the test.
        double prob = dataCruncher.getRiskOfFraudFigure(dataCruncher.readAllTransactions().stream()
                .filter(transaction -> transaction.getCustomerId().equals("C1687101094")).collect(Collectors.toList()).get(0));
        assertEquals(.49,prob,.1);
    }
}